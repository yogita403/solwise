...
class ImportForm extends FormBase {
  ...
  function saveHTMLtoProduct($content, $file, $type) {
    ...
    $imageviewer_content = $this->getContentBetweenHTMLComment($content, '<!-- InstanceBeginEditable name="imageViewer" -->', '<!-- InstanceEndEditable -->');
    if (!empty($imageviewer_content)) {
      $doc = new \DOMDocument();
      $doc->loadHTML($imageviewer_content);
      ...
      foreach ($imagelist as $key => $item) {
        ...
        if (!empty($image)) {
          ...
          if (isset($components['host'])) {
            ...
            $image = $this->solwise_url . $image;
          }
          // Checking the response code for the headshot url request.
          // $handle = curl_init($image);
          // curl_setopt($handle,  CURLOPT_RETURNTRANSFER, TRUE);
          // $response = curl_exec($handle);
          // $httpCode = curl_getinfo($handle, CURLINFO_HTTP_CODE);
          // if ($httpCode == 200) {
            $imageviewver = file_get_contents($image);
            $actual_name = pathinfo($image, PATHINFO_FILENAME);
            $extension = pathinfo($image, PATHINFO_EXTENSION);
            $img_file = file_save_data($imageviewver, 'public://product/' . $actual_name . '.' . $extension, FILE_EXISTS_RENAME);
            $imageviewer_img[] = !empty($img_file) ? $img_file->id() : [];
          // }
        }
      }
      // kint($imageviewer_img); exit;
      $field_content['field_image'] = $imageviewer_img;
    }
    // For stockCode field.
    $stock_content = $this->getContentBetweenHTMLComment($content, '<!-- InstanceBeginEditable name="pageConfig" -->', '<!-- InstanceEndEditable -->');
    $stock_content = $this->getContentBetweenSCRIPT($stock_content);
    if (!empty($stock_content)) {
      if (!empty($stock_content[1])) {
        $stockCode = $this->removeJSComments($stock_content[1][0]);
        $stockCode = trim($this->get_string_between($stockCode, "(", ")"));
        if ($stockCode != '' && $stockCode != 'null') {
          $stockCode = str_replace(";", "", $stockCode);
          $stockCode = str_replace("'", "", $stockCode);
          $stockCode = str_replace("\"", "", $stockCode);
          $field_content['field_associated_stockcode'] = explode(',', $stockCode);
        }
      }
    }
